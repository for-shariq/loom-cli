...
### Step 2 — Identify context surfaces
...
1. `scripts/scan_repo_context.py` — inventory all context files
2. `scripts/analyze_context_size.py` — classify always-on vs on-demand
3. `scripts/estimate_prompt_tokens.py <file>` — per-file estimate
4. `scripts/session_cost_estimator.py` — cost estimate for typical session
5. `scripts/generate_observability_report.py` — produce final report artifact
